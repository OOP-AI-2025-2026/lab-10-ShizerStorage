...
//        // Завдання 8
    ...
    public List<Order> getOrdersWithBabyProducts() {
        return orders.stream()
                .filter(order -> order.getProducts()
                        ...
                        .anyMatch(obj -> {
                            if (obj != null) {
                                return "Baby".equals(((Product) obj).getCategory());
                            }
                            return false;
                        }))
                .collect(Collectors.toList());
    ...
    public List<Product> applyDiscountToToys() {
        ...
        return products.stream()
                ...
                .map(p -> {
                    double newPrice = Math.round(p.getPrice() * 0.5 * 1000.0) / 1000.0;
                    return new Product(p.getId(), p.getName(), p.getCategory(), newPrice);
                })
                .collect(Collectors.toList());
    }
    ...
    public Map<String, List<Integer>> getProductsByCategory() {
        return products.stream()
                .collect(Collectors.groupingBy(Product::getCategory,
                        Collectors.mapping(Product::getId,
